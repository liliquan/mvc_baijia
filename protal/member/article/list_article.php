...
if(isset($_GET['keyword'])){
	...
	$likeSty = " and (title like '%{$keyword}%' or tags like '%{$keyword}%' or description like '%{$keyword}%' or article_content like '%{$keyword}%')";
}


$author_id = $_SESSION['author_id'];
$nowPage = isset($_GET['page'])?$_GET['page']:1;
$sql = "select id from baijia_article where author_id=".$author_id.$likeSty;
...
$sql = "select * from baijia_article where author_id= ".$author_id.$likeSty." order by id desc";
...
<html class=" js csstransforms3d"><head>
	...
	<script type="text/javascript">
		//ajax异步保存置顶状态	
		function saveIsTopState(article_id,istop){
				
		$.post("ajax_saveIsTopState.php",{"istop":istop,"article_id":article_id},function(reText){
				
				if(reText*1){
					
					alert("操作成功！");
				   if(istop){
						$("#Top"+article_id).attr("onClick","javascript:saveIsTopState("+article_id+",0)");
						$("#Top"+article_id).html("取消置顶");
					  
					}else{
						$("#Top"+article_id).attr("onClick","javascript:saveIsTopState("+article_id+",1)");
						$("#Top"+article_id).html("置顶");
					...
					<div class="Mian-cont-wrap">
						<div class="defaultTab-T">
							<table border="0" cellspacing="0" cellpadding="0" class="defaultTable">
								<tbody><tr><th class="t_1">文章ID</th><th class="t_2">文章标题</th><th class="t_3">发布时间</th><th class="t_3">操作</th></tr>
							</tbody></table>
						</div>
						<table border="0" cellspacing="0" cellpadding="0" class="defaultTable defaultTable2">
							...
							<tr>
								...
								<td class="t_2"><a href="protal/article_detail.php?id=1" target="_blank" mon="col=13&pn=1&a=12"><?php echo $articleArrs[$i]['title']; ?></a></td>
								<td class="t_3"><?php echo $articleArrs[$i]['add_time']; ?></td>
								<td class="t_4"><div class="btn">
								<?php if($articleArrs[$i]['is_top']==1){?>
								<a class="Top" id="Top1" onClick="javascript:saveIsTopState(1,0)">取消置顶</a>
								<?php }else{ ?>
								
								<a class="Top" id="Top1" onClick="javascript:saveIsTopState(1,1)">置顶</a>
								<?php } ?>
						...
						<div class="pageSelect">
							<span>共 <b><?php echo $totalCount ?></b> 条 每页 <b><?php echo PAGESIZE ?></b>条   <?php echo $nowPage."/".$pages ?></span>
							<div class="pageWrap">
								...
								<a href="list_article.php?page=<?php echo $i ?>&keyword=<?php echo "$keyword" ?>" class="pagenumb 'cur'"><?php echo $i ?></a>
								<?php }
								if($nowPage!=$pages){
								 ?>
