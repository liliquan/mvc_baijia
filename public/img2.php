<?php 
	...
	$img = imagecreatetruecolor($imgWidth, $imgHight);
	$backColor = imagecolorallocate($img, rand(0,150), rand(0,150), rand(0,150));
	imagefill($img, 0, 0, $backColor);

	$font = 16;
	$fontWidth = imagefontwidth($font);
	$fontHight = imagefontheight($font);
	...
	for($i=1;$i<=$n;$i++){
		$x = $space*$i;
		$y = mt_rand($font,($imgHight - $fontHight));
		...
		imagettftext($img, $font, mt_rand(-30,60), $x, $y+8, $fontColor, 'msyh.ttc', $str[$i-1]);
	}

	// 干扰线条
	for($i=0;$i<=5;$i++){
		$lineColor = imagecolorallocate($img, rand(0,150),rand(0,150),rand(0,150));
		imageline($img, rand(0,$imgWidth), rand(0,$imgHight), rand(0,$imgWidth), rand(0,$imgHight), $lineColor);
	}

	// 噪点
	for($i=0;$i<($imgWidth*$imgHight)*0.05;$i++){ 
		$pointColor = imagecolorallocate($img, rand(0,150),rand(0,150),rand(0,150));
		imagesetpixel($img,rand(0,$imgWidth),rand(0,$imgHight),$pointColor);
	}
